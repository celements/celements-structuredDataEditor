...
public class StructDataScriptService implements ScriptService {
  ...
  public String renderTable(DocumentReference cellDocRef) {
    ...
    if (rightsAccess.hasAccessLevel(cellDocRef, EAccessLevel.VIEW)) {
      ...
      Optional<ITablePresentationType> presentation = tableCfg
          .map(TableConfig::getType)
          .map(this::getPresentationType);
      if (tableCfg.isPresent() && presentation.isPresent()) {
        presentation.get().writeNodeContent(writer, cellDocRef, tableCfg.get());
      } else {
        writer.openLevel();
